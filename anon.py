...
# (тот же синтаксис форматирования, что и обычный parse_mode, плюс новые
# рич-возможности вроде нативных таблиц и заголовков через "#"), либо
# явное дерево блоков ("blocks"). Мы используем текстовый вариант —
# у нас уже есть готовый MarkdownV2-текст из esc()/quote()/header(), и не
# нужно строить точное JSON-дерево блоков, схема которого на момент
# написания ещё не до конца стабилизировалась в публичной документации.
...
async def send_rich_message(chat_id, markdown_text: str, reply_markup=None, disabled_callbacks=None):
    ...
    payload = {"chat_id": chat_id, "rich_message": {"markdown": markdown_text}}
    if reply_markup is not None:
        payload["reply_markup"] = disable_buttons(reply_markup, disabled_callbacks)
    data = await _rich_api_call("sendRichMessage", payload)
    if data.get("ok"):
        result = data.get("result") or {}
        if "message_id" in result:
...
async def edit_rich_message(chat_id, message_id, markdown_text: str, reply_markup=None, disabled_callbacks=None) -> bool:
    """editMessageText(rich_message=...) — True при успехе."""
    payload = {"chat_id": chat_id, "message_id": message_id, "rich_message": {"markdown": markdown_text}}
    if reply_markup is not None:
        payload["reply_markup"] = disable_buttons(reply_markup, disabled_callbacks)
    data = await _rich_api_call("editMessageText", payload)
    if data.get("ok"):
...
async def send_rich_or_plain(bot, chat_id, text: str, parse_mode: str = 'MarkdownV2', reply_markup=None):
    ...
    send_message. Возвращает отправленный message_id.
    """
    if RICH_MESSAGES_ENABLED and parse_mode in ('MarkdownV2', 'HTML'):
        payload = {"chat_id": chat_id}
        payload["rich_message"] = {"markdown": text} if parse_mode == 'MarkdownV2' else {"html": text}
        if reply_markup is not None:
...
async def show_screen(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str,
    ...
    if query:
        if use_rich and await edit_rich_message(chat_id, query.message.message_id, text, reply_markup, disabled_callbacks):
    ...
    if msg_id:
        if use_rich and await edit_rich_message(chat_id, msg_id, text, reply_markup, disabled_callbacks):
    ...
    if use_rich:
        rich_msg_id = await send_rich_message(chat_id, text, reply_markup, disabled_callbacks)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ─── Экономика (начислить/списать) — только админ, вместо /give и /take ───
            ...
            await show_screen(update, context, prompt, cancel_keyboard())
            return
