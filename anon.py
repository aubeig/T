...
def get_message_info(message_id):
    # ВАЖНО: сюда добавлен m.to_user_id — раньше его тут не было, и код
    # ниже (handle_text -> ответ на сообщение) по ошибке слал уведомление
    # на u_to.username (голый юзернейм без "@", а то и вовсе NULL) вместо
    # числового Telegram chat_id. Из-за этого отправка ВСЕГДА падала, и
    # ответы никогда не доходили до адресата.
    return run_query('''
        SELECT m.message_text, m.message_type, m.file_name, m.created_at,
               ...
               l.title as link_title, l.link_id, m.to_user_id
        FROM messages m
        LEFT JOIN users u_from ON m.from_user_id = u_from.user_id
        LEFT JOIN users u_to ON m.to_user_id = u_to.user_id
        LEFT JOIN links l ON m.link_id = l.link_id
...
BUTTON_LINE_RE = re.compile(r'^\[\[(.+?)\|(.+?)\]\]$', re.MULTILINE)


def extract_inline_buttons(text: str):
    """
    Позволяет админу добавлять кнопки-ссылки прямо в текст сообщения/
    рассылки строкой вида:
        [[Перейти на канал|https://t.me/channel]]
    Каждая такая строка выносится в отдельную inline-кнопку под
    сообщением, а из текста убирается. Возвращает (чистый_текст, [(label, url), ...]).
    """
    buttons = []
    if not text:
        return text, buttons
    for label, url in BUTTON_LINE_RE.findall(text):
        label = label.strip()
        url = url.strip()
        if not label or not url:
            continue
        # Telegram отклоняет кнопку целиком (Button_url_invalid), если у URL
        # нет схемы (https://, http://, tg://) — раньше это падало молча
        # где-то в глубине except Exception при отправке. Подставляем схему
        # сами, чтобы кнопка не ломала всю рассылку/уведомление.
        if not re.match(r'^(https?://|tg://)', url, re.IGNORECASE):
            url = 'https://' + url.lstrip('/')
        buttons.append((label[:64], url))
    clean_text = BUTTON_LINE_RE.sub('', text).strip()
        ...
        if data.startswith("admin_") or data in ("broadcast_send",):
            ...
            if data == "admin_panel":
                ...
                if context.user_data.get('broadcasting'):
                    ...
                    clean_text, link_buttons = extract_inline_buttons(message_text.strip())
                    safe_text = quote(esc(clean_text))
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Поддерживает кнопки-ссылки: строка [[Текст|https://...]] в тексте
        # превращается в отдельную inline-кнопку (например «Перейти на канал»).
        if context.user_data.get('admin_messaging_user'):
            target_user_id = context.user_data.pop('admin_messaging_user')
            clean_text, link_buttons = extract_inline_buttons(text)
            ...
            try:
                await context.bot.send_message(
                    target_user_id,
                    f"{SYM['write']} *Сообщение от администратора*\n\n{quote(esc(clean_text))}",
                    parse_mode='MarkdownV2',
                    reply_markup=keyboard
                )
                save_admin_message(user.id, target_user_id, clean_text)
                await show_screen(
                    update, context,
        ...
        if context.user_data.get('replying_to'):
            ...
            if message_info:
                ...
                msg_text, msg_type, file_name, created, from_user, from_name, to_user, to_name, link_title, link_id, to_user_id = message_info

                notification = f"{SYM['reply']} <b>Новый ответ на ваше сообщение</b>\n\n{quote_html(text_html)}"
                try:
                    if not to_user_id:
                        raise ValueError("to_user_id is empty — сообщение без адресата")
                    await context.bot.send_message(
                        to_user_id, notification, parse_mode='HTML',
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{SYM['reply']} Ответить", callback_data=f"reply_{message_id}")]])
                    )
                except Exception as e:
                    logging.error(f"Failed to send reply notification to {to_user_id}: {e}")
        ...
        if context.user_data.get('broadcasting') and is_admin:
            ...
            clean_preview, preview_buttons = extract_inline_buttons(text)
            btn_hint = f"\n{SYM['dot']} Кнопок\\-ссылок: {len(preview_buttons)}" if preview_buttons else ""
            await show_screen(
                update, context,
                f"{SYM['broadcast']} *Предпросмотр рассылки:*\n\n{quote(esc(clean_preview))}\n\n"
                f"{SYM['check']} Сообщение готово к отправке{esc('.')}{btn_hint}\n\n"
                f"{SYM['dot']} Чтобы добавить кнопку\\-ссылку, впишите строку `\\[\\[Текст|https://ссылка\\]\\]`",
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            if msg_type == 'photo':
                await context.bot.send_photo(link_info[1], file_id, caption=user_caption, parse_mode='HTML', reply_markup=message_actions_keyboard(msg_id))
            elif msg_type == 'video':
                await context.bot.send_video(link_info[1], file_id, caption=user_caption, parse_mode='HTML', reply_markup=message_actions_keyboard(msg_id))
            elif msg_type == 'document':
                await context.bot.send_document(link_info[1], file_id, caption=user_caption, parse_mode='HTML', reply_markup=message_actions_keyboard(msg_id))
            elif msg_type == 'voice':
                await context.bot.send_voice(link_info[1], file_id, caption=user_caption, parse_mode='HTML', reply_markup=message_actions_keyboard(msg_id))
            elif msg_type == 'video_note':
                await context.bot.send_video_note(link_info[1], file_id, reply_markup=message_actions_keyboard(msg_id))
                if caption:
                    await context.bot.send_message(link_info[1], f"{SYM['write']} <b>Подпись к кружку:</b>\n\n{quote_html(caption_html)}", parse_mode='HTML')
        except Exception as e:
