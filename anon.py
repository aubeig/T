...
async def send_rich_message(chat_id, markdown_text: str, reply_markup=None, disabled_callbacks=None, force_reply: bool = False):
    ...
    payload = {"chat_id": chat_id, "rich_message": {"markdown": markdownv2_to_rich_markdown(markdown_text)}}
...
async def edit_rich_message(chat_id, message_id, markdown_text: str, reply_markup=None, disabled_callbacks=None, force_reply: bool = False) -> bool:
    ...
    конвертируется в rich-markdown внутри — см. markdownv2_to_rich_markdown().
    force_reply — см. markup_to_payload() (force_reply, Bot API 10.3)."""
    payload = {"chat_id": chat_id, "message_id": message_id, "rich_message": {"markdown": markdownv2_to_rich_markdown(markdown_text)}}
    if reply_markup is not None:
        payload["reply_markup"] = markup_to_payload(reply_markup, disabled_callbacks, force_reply)
