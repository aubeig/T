...
FLOW_KEYS = (
    'creating_link', 'link_stage', 'link_title',
        ...
        if data == "main_menu":
            ...
            if links:
                ...
                for link in links:
                    ...
                    flat_buttons.append(InlineKeyboardButton(f"{SYM['trash']} {link[1][:20]}", callback_data=f"confirm_delete_link_{link[0]}"))
            ...
            if messages:
                ...
                for msg in messages:
                    ...
                    text += (
                        ...
                        f"`{esc_code(preview)}`\n"
                        f"{SYM['clock']} `{esc_code(created_str)}` {SYM['dot']} Ответов: {reply_count}\n\n"
                    )
            ...
            await show_screen(
                update, context,
                f"{SYM['reply']} *Ответ администратору*\n\nВведите текст ответа:",
        ...
        if data.startswith("admin_") or data in ("broadcast_send",):
            ...
            if data == "admin_panel":
                ...
                if context.user_data.get('broadcasting'):
                    ...
                    safe_text = quote(esc(message_text.strip()))
                    for u in users:
                        try:
                            await context.bot.send_message(
                                u[0],
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if context.user_data.get('replying_to_admin'):
            ...
            if ADMIN_ID:
                try:
                    reply_header = f"{SYM['reply']} *Ответ от пользователя* `{user.id}`"
                    if user.username:
                        reply_header += f" \\(@{esc(user.username)}\\)"
                    await context.bot.send_message(
                        ADMIN_ID,
                        f"{reply_header}\n\n{quote(esc(text))}",
                        parse_mode='MarkdownV2',
        ...
        if context.user_data.get('admin_messaging_user'):
            ...
            try:
                await context.bot.send_message(
                    ...
                    parse_mode='MarkdownV2',
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{SYM['reply']} Ответить", callback_data="user_reply_admin")]])
                )
        ...
        if context.user_data.get('current_link'):
            ...
            if link_info:
                ...
                sponsor_badge = f"{SYM['gift']} *через спонсорскую ссылку*\n" if link_info[5] else ""
...
REPORT_STYLE = '''
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { text-align: center; margin-bottom: 40px; animation: fadeInUp 0.6s ease both; }
        .title {
        ...
        .section h2 {
            font-weight: 800; font-size: 1.6em; margin-bottom: 22px; color: #ffffff; letter-spacing: 1px;
            display: flex; align-items: center; gap: 10px;
        }
        .section h2::before { content: ''; width: 8px; height: 8px; border-radius: 50%; background: #00F0FF; box-shadow: 0 0 10px #00F0FF; }
        ...
        .sponsor-tag { color: #FFD24A; font-weight: bold; }
        .message-block {
            background: rgba(255,255,255,0.04); padding: 18px; margin: 12px 0; border-radius: 14px;
        ...
        .footer { text-align: center; margin-top: 40px; padding: 26px; background: rgba(112,0,255,0.06); border-radius: 20px; border: 1px solid rgba(255,255,255,0.08); animation: fadeInUp 0.6s ease both; }
...
def generate_link_conversation_report(link_id, link_info=None):
    ...
    html_content += f'''
        <div class="header">
            <h1 class="title">Переписка по ссылке</h1>
            <p class="subtitle">{escape_html_safe(title)}{sponsor_tag}</p>
            <p style="color:#A0A0C0;">ID ссылки: {escape_html_safe(link_id)} · Сгенерирован: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} (KRA)</p>
        </div>
        <div class="section">
    '''
        ...
        for conv in conversations:
            if conv[0] == 'message':
                ...
                if conv[3] and conv[3] != 'text':
                    file_size = f" ({conv[5] // 1024} KB)" if conv[5] else ""
                    media_info = f'<div class="media-info">Тип: {conv[3].upper()}{file_size} · Файл: {escape_html_safe(conv[6]) or "без названия"}</div>'
