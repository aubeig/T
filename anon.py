...
#   ASR_RELAY_CHAT_ID — id владельца моста (по умолчанию 3811126444);
#   ASR_SALUTE_BOT    — бот-расшифровщик (по умолчанию smartspeech_sber_bot);
#   ASR_SALUTE_BOT_ID — его числовой ID (надёжнее юзернейма, тот не меняется);
...
def _asr_state_load() -> None:
    ...
    try:
        ...
        if row and row[0]:
            st = json.loads(row[0])
            for k in ("connection_id", "user_id", "salute_chat_id", "enabled"):
                if k in st:
                    _asr_bridge[k] = st[k]
...
def _asr_bridge_for(owner_id) -> bool:
    ...
    return bool(
        ...
        and _asr_bridge["salute_chat_id"]
    )
...
async def _notify_relay(context, text: str) -> None:
    ...
    if not ASR_RELAY_CHAT_ID:
        return
    try:
        await context.bot.send_message(ASR_RELAY_CHAT_ID, text)
    except Exception as e:
        logging.warning(f"ASR-мост: не уведомил владельца: {e}")
    ...
    try:
        ...
        if _asr_bridge["enabled"] and not was_enabled:
            await _notify_relay(
                ...
                f"Отправь боту @{ASR_SALUTE_BOT} любое сообщение (например «привет»), "
                f"чтобы я запомнил этот чат. После этого голосовые и кружки через "
                f"спонсорские ссылки будут расшифровываться полностью автоматически.",
            )
...
async def _handle_business_message(context, bmsg) -> None:
    ...
    try:
        ...
        if sender is None or not sender.is_bot:
            return  # свои сообщения владельца нас не интересуют
        # Опознаём бот-расшифровщик по ID (не меняется) или по юзернейму
        # (на случай, если ID по каким-то причинам не совпадает)
        if getattr(sender, "id", None) != ASR_SALUTE_BOT_ID and (sender.username or "").lower() != ASR_SALUTE_BOT:
            return
...
async def _send_job_to_salute(context, job) -> bool:
    """Отправляет аудио от имени владельца в его чат с бот-расшифровщиком."""
    f = await context.bot.get_file(job["file_id"])
    data = bytes(await f.download_as_bytearray())
    fields = {
        "chat_id": str(_asr_bridge["salute_chat_id"]),
    ...
    if r.get("ok"):
        ...
        logging.info(f"ASR-мост: задача #{job['id']} отправлена в чат Salute")
        return True
    desc = str(r.get("description") or "")
    logging.warning(f"ASR-мост: {method} не удался: {desc}")
    ...
    if "business_connection" in desc.lower() or "BUSINESS_CONNECTION" in desc:
...
async def _send_manual_card(context, job) -> None:
    ...
    try:
        if job["msg_type"] == "video_note":
            # у video_note нет подписи — инструкцию шлём отдельным сообщением
            m = await context.bot.send_video_note(ASR_RELAY_CHAT_ID, video_note=job["file_id"])
            _asr_manual_cards[m.message_id] = job["id"]
            try:
                m2 = await context.bot.send_message(
                    ASR_RELAY_CHAT_ID, inst, reply_to_message_id=m.message_id
                )
            ...
            m = await context.bot.send_voice(
                ASR_RELAY_CHAT_ID, voice=job["file_id"], caption=inst
            )
            _asr_manual_cards[m.message_id] = job["id"]
        logging.info(f"ASR-мост: карточка #{job['id']} отправлена владельцу (ручной режим)")
    except Exception as e:
        _asr_jobs.pop(job["id"], None)
        logging.warning(f"ASR-мост: карточка не отправилась: {e}")
