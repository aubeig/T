...
def admin_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{SYM['gift']} Спонсорские ссылки", callback_data="admin_sponsor_links")],
...
def sponsor_links_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{SYM['add']} Создать спонсорскую ссылку", callback_data="admin_create_sponsor_link")],
        [InlineKeyboardButton(f"{SYM['report']} Мои спонсорские ссылки", callback_data="admin_my_sponsor_links")],
...
def sponsor_link_actions_keyboard(link_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{SYM['transfer']} Передать ссылку", callback_data=f"admin_transfer_sponsor_{link_id}")],
        [InlineKeyboardButton(f"{SYM['trash']} Удалить ссылку", callback_data=f"admin_delete_sponsor_{link_id}")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if is_user_banned(user.id):
            await update.message.reply_text(f"{SYM['ban']} Вы заблокированы в этом боте и не можете использовать его функции.")
            return
        ...
        if not context.user_data.get('admin_authenticated'):
            if context.args and context.args[0] == ADMIN_PASSWORD:
                ...
                sent = await update.effective_chat.send_message(
                    f"{SYM['check']} *Панель администратора*",
            ...
            sent = await update.message.reply_text(
                f"{SYM['gear']} *Панель администратора*",
                reply_markup=admin_keyboard(),
                parse_mode='MarkdownV2'
            )
            context.user_data['screen_msg_id'] = sent.message_id
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "main_menu":
            ...
            if links:
                text = f"{SYM['link']} *Ваши анонимные ссылки:*\n\n"
                keyboard_buttons = []
                ...
                for link in links:
                    ...
                    text += (
                        ...
                        f"{esc(link[2])}\n"
                        f"`{esc(link_url)}`\n"
                        f"{SYM['clock']} `{esc(created)}`\n\n"
                    )
                    keyboard_buttons.append([InlineKeyboardButton(f"{SYM['trash']} {link[1][:24]}", callback_data=f"confirm_delete_link_{link[0]}")])
            ...
            if messages:
                text = f"{SYM['inbox']} *Ваши последние сообщения:*\n\n"
                keyboard_buttons = []
                for msg in messages:
                    ...
                    text += (
                        ...
                        f"`{esc(preview)}`\n"
                        f"{SYM['clock']} `{esc(created_str)}` {SYM['dot']} Ответов: {reply_count}\n\n"
                    )
        ...
        # ─── АДМИН ПАНЕЛЬ ───
        if data.startswith("admin_") or data in ("broadcast_send",):
            ...
            if data == "admin_panel":
                await show_screen(update, context, f"{SYM['gear']} *Панель администратора*", admin_keyboard())
                return

            elif data == "admin_stats":
                stats = get_admin_stats()
                text = (
                    f"{SYM['stats']} *Статистика бота*\n\n"
                    f"*Пользователи*\n"
                ...
                # поэтому экранируем только точки/восклицательные знаки в заголовках вручную:
                text = text.replace("*Статистика бота*", "*Статистика бота*").replace(":", "\\:")
                ...
                if users:
                    text = f"{SYM['users']} *Управление пользователями*\n\n"
                    keyboard_buttons = []
                    for u in users[:15]:
                        ...
                        text += f"*{esc(username_display)}*\n`{u[0]}` {SYM['dot']} `{esc(created)}` {SYM['dot']} {ban_status}\n\n"
                        keyboard_buttons.append([InlineKeyboardButton(f"{SYM['id']} {username_display}", callback_data=f"admin_user_manage_{u[0]}")])
                ...
                if user_info:
                    ...
                    text = (
                        f"{SYM['id']} *Управление пользователем*\n\n"
                        f"ID: `{user_id}`\n"
                ...
                if success:
                    try:
                        ...
                        pass
                    await show_screen(update, context, f"{SYM['check']} Пользователь `{user_id}` разблокирован{esc('!')}", user_management_keyboard(user_id, False))
                else:
                ...
                await show_screen(
                    update, context,
                    f"{SYM['gift']} *Создание спонсорской ссылки*\n\nВведите *название*:",
                ...
                if sponsor_links:
                    text = f"{SYM['gift']} *Ваши спонсорские ссылки:*\n\n"
                    keyboard_buttons = []
                    bot_username = context.bot.username
                    for link in sponsor_links:
                        ...
                        created_str = format_datetime(created)
                        custom_info = f"\nID: `{esc(custom_id)}`" if custom_id else ""
                        text += (
                            f"*{esc(title)}*\n{esc(description)}\n"
                            f"Владелец: `{target_user_id}`{custom_info}\n"
                            f"`{esc(link_url)}`\n{SYM['clock']} `{esc(created_str)}`\n\n"
                        )
                        keyboard_buttons.append([InlineKeyboardButton(f"{SYM['transfer']} {title[:24]}", callback_data=f"admin_sponsor_actions_{link_id}")])
                ...
                if link_info:
                    text = (
                        f"{SYM['gift']} *Управление спонсорской ссылкой*\n\n"
                        f"Название: {esc(link_info[2])}\n"
                        f"Описание: {esc(link_info[3])}\n"
                        f"ID: `{link_id}`"
                    )
                ...
                await show_screen(
                    update, context,
                    f"{SYM['transfer']} *Передача спонсорской ссылки*\n\nВведите *ID пользователя*:",
                ...
                await show_screen(
                    update, context,
                    f"{SYM['broadcast']} *Режим рассылки*\n\nВведите сообщение для рассылки всем пользователям:",
                ...
                if context.user_data.get('broadcasting'):
                    ...
                    safe_text = esc(message_text.strip())
                    for u in users:
                        try:
                            await context.bot.send_message(
                                u[0],
                                f"{SYM['broadcast']} *Оповещение от администратора*\n\n{safe_text}",
                                parse_mode='MarkdownV2'
                            )
                            success_count += 1
                    ...
                    await show_screen(
                        update, context,
                        f"{SYM['check']} *Рассылка завершена{esc('!')}*\n\n"
                ...
                if user_links:
                    ...
                    for link in user_links:
                        created = format_datetime(link[3])
                        text += f"*{esc(link[1])}*\n{esc(link[2])}\n{SYM['clock']} `{esc(created)}` {SYM['dot']} Сообщений: {link[4]}\n\n"
                    keyboard = InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"{SYM['view']} Посмотреть переписку", callback_data=f"admin_view_conversation_{user_id}")],
                        [InlineKeyboardButton(f"{SYM['back']} Назад", callback_data=f"admin_user_manage_{user_id}")]
                    ])
                    await show_screen(update, context, text, keyboard)
                else:
    ...
    try:
        ...
        if context.user_data.get('admin_messaging_user'):
            ...
            try:
                await context.bot.send_message(
                    target_user_id,
                    f"{SYM['write']} *Сообщение от администратора*\n\n{esc(text)}",
                    parse_mode='MarkdownV2'
                )
        ...
        if context.user_data.get('creating_sponsor_link'):
            ...
            if stage == 'title':
                ...
                if custom_id in ('-', ''):
                    ...
                    await show_screen(
                        update, context,
                        f"{SYM['warn']} Кастомный ID может содержать только буквы, цифры, `_` и `-` \\(3\\-32 символа\\){esc('.')} Попробуйте снова:",
                        InlineKeyboardMarkup([[InlineKeyboardButton(f"{SYM['cancel']} Отмена", callback_data="admin_sponsor_links")]])
                    )
                    return
                ...
                try:
                    ...
                    if link_id is None:
                        await show_screen(
                            update, context,
                            f"{SYM['warn']} Кастомный ID уже занят{esc('!')} Попробуйте другой{esc('.')}",
                            sponsor_links_keyboard()
                        )
                        return
                    ...
                    await show_screen(
                        update, context,
                        f"{SYM['check']} *Спонсорская ссылка создана{esc('!')}*\n\n"
                        f"*{esc(title)}*\n{esc(description)}\n"
        ...
        if context.user_data.get('replying_to'):
            ...
            if message_info:
                ...
                notification = f"{SYM['reply']} *Новый ответ на ваше сообщение*\n\n{esc(text)}"
                try:
        ...
        if context.user_data.get('creating_link'):
            ...
            if stage == 'title':
                ...
                await show_screen(
                    update, context,
                    f"{SYM['check']} *Ссылка создана{esc('!')}*\n\n"
                    f"*{esc(title)}*\n{esc(text)}\n\n"
        ...
        if context.user_data.get('broadcasting') and is_admin:
            ...
            await show_screen(
                update, context,
                f"{SYM['broadcast']} *Предпросмотр рассылки:*\n\n{esc(text)}\n\n"
                f"{SYM['check']} Сообщение готово к отправке{esc('.')}",
                InlineKeyboardMarkup([
                    [InlineKeyboardButton(f"{SYM['broadcast']} Отправить", callback_data="broadcast_send")],
                    [InlineKeyboardButton(f"{SYM['write']} Редактировать", callback_data="admin_broadcast")]
                ])
            )
            return
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if file_name:
            file_info += f"\n{SYM['file']} `{esc(file_name)}`"

        icon = TYPE_ICON.get(msg_type, SYM['doc'])
        user_caption = f"{icon} *Новый анонимный файл*{file_info}\n\n{esc(caption)}"
        ...
        try:
            if msg_type == 'photo':
                ...
                if caption:
                    await context.bot.send_message(link_info[1], f"{SYM['write']} *Подпись к кружку:*\n\n{esc(caption)}", parse_mode='MarkdownV2')
        except Exception as e:
...
REPORT_STYLE = '''
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { max-width: 1400px; margin: 0 auto; min-width: 1000px; }
        .header { text-align: center; margin-bottom: 40px; }
        .title {
            font-weight: 900; font-size: 3em;
            background: linear-gradient(135deg, #7000FF 0%, #8A2BE2 50%, #00F0FF 100%);
            -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
            margin-bottom: 15px; text-transform: uppercase; letter-spacing: 2px;
        }
        .subtitle { font-weight: 600; font-size: 1.2em; color: #a78bfa; opacity: 0.9; margin-bottom: 20px; }
        .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 20px; margin-bottom: 35px; }
        .stat-card {
            background: linear-gradient(135deg, rgba(112,0,255,0.08) 0%, rgba(0,240,255,0.04) 100%);
            backdrop-filter: blur(15px); padding: 28px 22px; border-radius: 20px; text-align: center;
            border: 1px solid rgba(255,255,255,0.08); transition: all 0.3s ease;
        }
        .stat-card:hover { transform: translateY(-6px); border-color: rgba(0,240,255,0.35); }
        .stat-card h3 {
            font-weight: 800; font-size: 2.6em; margin-bottom: 10px;
            background: linear-gradient(135deg, #00F0FF 0%, #8A2BE2 100%);
            -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
        }
        .stat-card p { color: #A0A0C0; font-size: 0.95em; font-weight: 500; text-transform: uppercase; letter-spacing: 1px; }
        .section {
            background: rgba(255,255,255,0.03); backdrop-filter: blur(15px);
            padding: 28px; border-radius: 20px; margin-bottom: 30px;
        ...
        .section h2 { font-weight: 800; font-size: 1.6em; margin-bottom: 22px; color: #ffffff; letter-spacing: 1px; }
        table { width: 100%; border-collapse: collapse; background: rgba(255,255,255,0.015); border-radius: 14px; overflow: hidden; margin-top: 12px; }
        th, td { padding: 13px 18px; text-align: left; border-bottom: 1px solid rgba(255,255,255,0.05); }
        th {
            background: linear-gradient(135deg, rgba(112,0,255,0.18) 0%, rgba(0,240,255,0.1) 100%);
            color: #00F0FF; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; font-size: 0.85em;
        }
        td { color: #dcdcf5; font-weight: 500; font-size: 0.95em; }
        tr:hover { background: rgba(255,255,255,0.04); }
        .user-banned { color: #ff5c7a; font-weight: bold; }
        .user-active { color: #00F0FF; font-weight: bold; }
        .message-block { background: rgba(255,255,255,0.04); padding: 18px; margin: 12px 0; border-radius: 14px; border-left: 3px solid #8A2BE2; }
        .message-block:hover { background: rgba(255,255,255,0.07); }
        .message-block.reply { border-left-color: #00F0FF; margin-left: 36px; }
        .message-header { display: flex; justify-content: space-between; margin-bottom: 8px; font-weight: 600; color: #00F0FF; font-size: 0.9em; }
        .message-content { color: #dcdcf5; line-height: 1.5; word-break: break-word; }
        .timestamp { color: #A0A0C0; font-size: 0.85em; }
        .media-info { background: rgba(112,0,255,0.08); padding: 8px 12px; border-radius: 8px; margin-top: 8px; font-size: 0.85em; border-left: 2px solid #8A2BE2; }
        .footer { text-align: center; margin-top: 40px; padding: 26px; background: rgba(112,0,255,0.06); border-radius: 20px; border: 1px solid rgba(255,255,255,0.08); }
        .footer-text { font-weight: 800; font-size: 1em; color: #00F0FF; letter-spacing: 2px; }
'''
...
def generate_conversation_report(user_id):
    """Генерирует HTML-отчёт переписки пользователя."""
    ...
    html_content += f'''
        <div class="header">
            <h1 class="title">Переписка · {user_id}</h1>
            <p class="subtitle">Полная история сообщений и ответов</p>
            <p style="color:#A0A0C0;">Сгенерирован: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} (KRA)</p>
        </div>
        <div class="section">
    '''
    ...
    if conversations:
        for conv in conversations:
            if conv[19] == 'message':
                ...
                if conv[2] and conv[2] != 'text':
                    ...
                    media_info = f'<div class="media-info">Тип: {conv[2].upper()}{file_size} · Файл: {escape_html_safe(conv[5]) or "без названия"}</div>'
        ...
        html_content += '<p style="color:#A0A0C0;">Нет данных о переписке</p>'
...
def generate_beautiful_html_report():
    ...
    html_content += f'''
        <div class="header">
            ...
            <div style="color:#A0A0C0;">Отчёт сгенерирован: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} (KRA)</div>
        </div>
    ...
    for user in data['users'][:30]:
        ...
        status = '<span class="user-banned">ЗАБЛОКИРОВАН</span>' if user[4] else '<span class="user-active">АКТИВЕН</span>'
