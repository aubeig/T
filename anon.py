...
def main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton(f"{SYM['shop']} Магазин", callback_data="shop_menu"),
        ]
    ])


def shop_keyboard(balance: int):
    can_afford = balance >= SPONSOR_LINK_PRICE
    label = f"{SYM['gift']} Спонсорская ссылка — {SPONSOR_LINK_PRICE} виол"
    if not can_afford:
        label += f" {SYM['lock']}"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(label, callback_data="shop_buy_sponsor_link")],
...
# ══════════════════════════════════════════════════════════════════
#  АДМИН-КОМАНДЫ ЭКОНОМИКИ (баланс: выдать / отнять / посмотреть)
# ══════════════════════════════════════════════════════════════════

def _parse_user_and_amount(args, need_amount=True):
    """
    Разбирает аргументы команды вида "<user_id> [amount]".
    Возвращает (user_id: int, amount: int|None, error: str|None).
    """
    if not args or len(args) < (2 if need_amount else 1):
        return None, None, "not_enough_args"
    user_id = safe_int(args[0], default=None)
    if user_id is None:
        return None, None, "invalid_user_id"
    if not need_amount:
        return user_id, None, None
    amount = safe_int(args[1], default=None)
    if amount is None or amount <= 0:
        return user_id, None, "invalid_amount"
    return user_id, amount, None
...
async def give_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/give <user_id> <amount> — начислить виолы пользователю (только админ)."""
    try:
        user = update.effective_user
        if not is_admin_user(user):
            await update.message.reply_text(f"{SYM['lock']} *Доступ запрещён*", parse_mode='MarkdownV2')
        ...
        target_id, amount, err = _parse_user_and_amount(context.args)
        if err == "not_enough_args":
            await update.message.reply_text(
                f"Использование: `/give <user_id> <amount>`",
                parse_mode='MarkdownV2'
            )
            return
        ...
        if err == "invalid_amount":
            await update.message.reply_text(f"{SYM['warn']} Сумма должна быть положительным целым числом\\.", parse_mode='MarkdownV2')
            return
        ...
        try:
            ...
            await update.message.reply_text(f"{SYM['warn']} Не удалось начислить\\. Буфер недоступен\\.", parse_mode='MarkdownV2')
            return
        ...
        logging.error(f"Ошибка в команде give: {e}")
        try:
            await update.message.reply_text(f"{SYM['warn']} Произошла ошибка\\. Попробуйте позже\\.", parse_mode='MarkdownV2')
        except Exception:
            pass
...
async def take_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/take <user_id> <amount> — списать виолы у пользователя (только админ)."""
    try:
        user = update.effective_user
        if not is_admin_user(user):
            await update.message.reply_text(f"{SYM['lock']} *Доступ запрещён*", parse_mode='MarkdownV2')
        ...
        if err == "not_enough_args":
            await update.message.reply_text(
                f"Использование: `/take <user_id> <amount>`",
                parse_mode='MarkdownV2'
            )
            return
        if err == "invalid_user_id":
        ...
        if not ok:
            if debit_err == "insufficient_balance":
                ...
                await update.message.reply_text(f"{SYM['warn']} Не удалось списать\\. Буфер недоступен\\.", parse_mode='MarkdownV2')
            return
        ...
        logging.error(f"Ошибка в команде take: {e}")
        try:
            await update.message.reply_text(f"{SYM['warn']} Произошла ошибка\\. Попробуйте позже\\.", parse_mode='MarkdownV2')
        except Exception:
            pass
    ...
    try:
        ...
        if data == "main_menu":
            ...
            try:
                balance = await asyncio.to_thread(vb.get_balance, user.id)
                text = (
                    header("Магазин", SYM['shop']) + "\n\n"
                    f"{SYM['coin']} Ваш баланс: *{balance}* виол\n\n"
                    f"Здесь можно купить спонсорскую ссылку за виолы{esc('.')}"
                )
                await show_screen(update, context, text, shop_keyboard(balance))
            except vb.BufferError as e:
                logging.error(f"Ошибка получения баланса для магазина user_id={user.id}: {e}")
                await show_screen(update, context, f"{SYM['warn']} Магазин временно недоступен\\.", main_keyboard())
            return

        elif data == "shop_buy_sponsor_link":
            # Идемпотентность покупки: если пользователь дважды нажмёт кнопку до того,
            # как обработается первое нажатие, второй debit с тем же ключом просто
            # вернёт duplicate=True в буфере и не спишет дважды. Ключ живёт короткое
            # время (по времени в секундах) — специально НЕ делаем его полностью
            # уникальным на каждый клик, чтобы защититься от двойного тапа.
            purchase_key = f"buy_sponsor_link_{user.id}_{int(time.time())}"
            ok, balance, err = await asyncio.to_thread(
                vb.try_debit, user.id, SPONSOR_LINK_PRICE,
                {"reason": "buy_sponsor_link"}, purchase_key
            )
            if not ok:
                ...
                await show_screen(update, context, text, shop_keyboard(balance or 0))
                return

            # Списание прошло — создаём ссылку. Покупатель = владелец своей спонсорской ссылки.
            link_id = create_anon_link(
                user.id, "Спонсорская ссылка", "Спонсорская анонимная ссылка",
                is_sponsor=True, sponsor_owner_id=user.id
            )
            if not link_id:
                ...
                try:
                    await asyncio.to_thread(
                        ...
                        {"reason": "refund_failed_sponsor_link_purchase"}
                    )
                except vb.BufferError as e:
                    logging.critical(f"КРИТИЧНО: не удалось вернуть виолы user_id={user.id} после неудачной покупки: {e}")
                text = f"{SYM['warn']} Не удалось создать ссылку\\. Виолы возвращены на баланс\\."
                await show_screen(update, context, text, main_keyboard())
                return
            ...
            text = (
                header("Покупка успешна", SYM['check']) + "\n\n"
                f"{SYM['gift']} Спонсорская ссылка создана\\.\n"
                f"Списано: *{SPONSOR_LINK_PRICE}* виол\\.\n"
                f"Остаток: *{balance}* виол\n\n"
                f"`{esc_code(link_url)}`"
            )
            await show_screen(update, context, text, main_keyboard())
            return
