...
# ══════════════════════════════════════════════════════════════════
#  РАСШИФРОВКА ЧЕРЕЗ БОТА @smartspeech_sber_bot (мост Telegram Business)
# ══════════════════════════════════════════════════════════════════
# Боты в Telegram НЕ умеют общаться напрямую (не пишут и не слышат друг
# друга), поэтому расшифровка через бот Salute делается мостом. Владелец
# (ASR_RELAY_CHAT_ID) подключает нашего бота как чат-бота Telegram Business
# (Настройки → Для бизнеса → Чат-боты — бесплатно), после чего бот видит
# личные чаты владельца (update business_message) и умеет писать ОТ ЕГО
# ИМЕНИ (business_connection_id в send-методах). Дальше всё автоматически:
#   1. голосовое/кружок уходит от имени владельца в его чат с
#      @smartspeech_sber_bot;
#   2. служебное «Аудиосообщение принято!» пропускается;
#   3. текст расшифровки забирается, доставляется получателю и подшивается
#      в БД (виден в «Моих сообщениях» и отчётах);
#   4. служебные сообщения в чате подметаются (deleteMessages).
# Если бизнес-мост ещё не подключён — задача уходит «карточкой» в чат
# владельца: голосовое + инструкция; расшифровка принимается ответом
# (reply) на карточку, служебные фразы Salute тоже отсекаются.
#   ASR_RELAY_CHAT_ID — чат владельца для карточек/уведомлений (текущий ID
#                       группового чата 3811126444 подходит, но ЕСЛИ ПРИДЁТ
#                       «chat not found» — значит нужен минус: -3811126444;
#                       бот пробует оба варианта и запоминает рабочий);
#   ASR_SALUTE_BOT    — бот-расшифровщик (по умолчанию smartspeech_sber_bot);
#   ASR_SALUTE_BOT_ID — его числовой ID. Это также id ЛИЧНОГО чата владельца
#                       с Салютом — туда (от имени владельца, через business)
#                       уходит аудио. Групповые чаты бизнес-боту недоступны!
#   ASR_JOB_TIMEOUT   — сколько секунд ждать ответа Salute (потом задача
#                       переотправляется карточкой для ручной расшифровки).
#
# ЧЕК-ЛИСТ АВТОМАТИЧЕСКОЙ РАСШИФРОВКИ (всё бесплатно, 2 минуты):
#   1. Настройки → Для бизнеса → Чат-боты → добавить нашего бота,
#      разрешения: «Существующие чаты» + ответы/удаление сообщений;
#   2. @BotFather → /mybots → НАШ бот → Bot Settings →
#      Bot-to-Bot Communication Mode → Enable (без этого Telegram не даёт
...
def init_db():
    try:
        ...
        # key-value хранилище мелкого состояния бота (например, состояние
        # ASR-моста: connection_id/чат Salute), чтобы переживать рестарты
        cursor.execute('''
...
def get_message_asr_info(message_id):
    """Данные для расшифровки по кнопке: тип, файл, получатель, текущий текст."""
    return run_query(
        'SELECT message_type, file_id, to_user_id, message_text FROM messages WHERE message_id = ?',
        (message_id,), fetch="one"
    )
...
def rich_button(text, data=None, url=None, style="primary", disabled=False):
    """
    ...
    disabled=True рендерит <tg-button type="disabled"> — кнопка видна, но
    не нажимается (например, «Купить», когда не хватает виол).
    """
    label = esc_html(text)
    if disabled:
...
async def send_rich_or_plain(bot, chat_id, text: str, parse_mode: str = 'MarkdownV2', reply_markup=None, body_buttons=None):
    """
    ...
    body_buttons — список строк от rich_button_row(): цветные кнопки в теле
    сообщения (RichBlockButtons, Bot API 10.3). На rich-пути они добавляются
    в конец тела и заменяют reply_markup; reply_markup остаётся только для
    MarkdownV2/HTML-fallback.
    """
...
async def send_rich_draft(chat_id, draft_id, markdown_text) -> bool:
    """
    sendRichMessageDraft (Bot API 10.1+) — эфемерный «живой» черновик,
    который виден пользователю пока идёт длинная операция (например,
    расшифровка голосового через Salute). Работает ТОЛЬКО в приватных чатах;
    черновик живёт ~30 секунд и пропадает сам, поэтому его используют как
    анимируемый статус-индикатор, а не как постоянное сообщение.
    Возвращает True при успехе.
    """
    if not RICH_MESSAGES_ENABLED:
        return False
    payload = {"chat_id": chat_id, "draft_id": draft_id, "rich_message": {"markdown": markdown_text}}
    data = await _rich_api_call("sendRichMessageDraft", payload)
...
async def _transcribe_with_streaming(context, chat_id, file_id, msg_type):
    """
    Скачивает голосовое/кружок, запускает фоновый статус-черновик
    (стриминг «Расшифровываю…») и распознаёт текст через Salute.
    Возвращает текст или None. Все ошибки глотаются с логом — флоу
    отправки медиа продолжается в любом случае.
    """
    if not salute_asr.enabled():
        return None
    stream_task = None
    if RICH_MESSAGES_ENABLED:
        stream_task = asyncio.create_task(_stream_asr_status(context, chat_id))
    try:
        f = await context.bot.get_file(file_id)
        data = await f.download_as_bytearray()
        transcript = await salute_asr.transcribe_media(bytes(data), msg_type)
        if transcript:
            transcript = transcript.strip()
        return transcript or None
    except Exception as e:
        logging.warning(f"Расшифровка {msg_type} через Salute не удалась: {e}")
        return None
    finally:
        if stream_task:
            stream_task.cancel()
...
# ══════════════════════════════════════════════════════════════════
#  МОСТ РАСШИФРОВКИ: наш бот ↔ владелец ↔ @smartspeech_sber_bot
# ══════════════════════════════════════════════════════════════════

# Состояние моста (живёт в памяти процесса; чат Salute запоминается, когда
# владелец пишет/пересылает туда что-нибудь — после рестарта достаточно
# кинуть Salute-боту «привет» ещё раз).
_asr_bridge = {
    "connection_id": None,   # business_connection_id
    "user_id": None,         # владелец бизнес-подключения
    "salute_chat_id": None,  # чат владельца с бот-расшифровщиком
    "enabled": False,        # is_enabled у BusinessConnection
}
_asr_jobs = {}           # job_id -> задача расшифровки
_asr_manual_cards = {}   # message_id карточки в чате владельца -> job_id
_asr_queue = []          # id задач в порядке поступления
_asr_pump_running = False
_asr_job_seq = 0


def _asr_state_save() -> None:
    """
    Сохраняем состояние моста в БД (таблица bot_state, уезжает на GitHub
    вместе с data.db) — чтобы после рестарта/пересборки хостинга мост
    продолжал работать без участия владельца.
    """
    try:
        run_query(
            "INSERT INTO bot_state (key, value) VALUES ('asr_bridge', ?) "
            "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
            (json.dumps(_asr_bridge, ensure_ascii=False),),
            commit=True,
        )
        push_db_to_github("ASR bridge state updated")
    except Exception as e:
        logging.info(f"ASR-мост: состояние не сохранено: {e}")


def _asr_state_load() -> None:
    """Восстанавливаем состояние моста после старта процесса."""
    try:
        row = run_query("SELECT value FROM bot_state WHERE key = 'asr_bridge'", fetch="one")
        if row and row[0]:
            st = json.loads(row[0])
            for k in ("connection_id", "user_id", "salute_chat_id", "enabled", "relay_chat_id"):
                if k in st:
                    _asr_bridge[k] = st[k]
            if _asr_bridge.get("enabled"):
                logging.info(
                    f"ASR-мост: состояние восстановлено из БД "
                    f"(connection_id={_asr_bridge.get('connection_id')}, "
                    f"salute_chat={_asr_bridge.get('salute_chat_id')})"
                )
    except Exception as e:
        logging.info(f"ASR-мост: состояние не восстановлено: {e}")

# Служебные ответы Salute, которые надо ПРОПУСКАТЬ (не принимать за текст)
_ASR_PLACEHOLDERS = (
    "аудиосообщение принято",
    "аудио сообщение принято",
    "аудиосообщение получено",
    "голосовое сообщение принято",
    "голосовое получено",
)
_ASR_PLACEHOLDER_NORMS = tuple(re.sub(r"[!.,\s»«]+", "", p.lower()) for p in _ASR_PLACEHOLDERS)


def _norm_asr_text(text: str) -> str:
    """Нормализация для отсечения служебных фраз Salute."""
    return re.sub(r"[!.,\s»«]+", "", (text or "").lower())


def _asr_bridge_for(owner_id) -> bool:
    """Мост готов и принадлежит именно этому владельцу ссылки."""
    return bool(
        ASR_RELAY_CHAT_ID
        and _asr_bridge["enabled"]
        and _asr_bridge["connection_id"]
        and _asr_bridge["user_id"] == owner_id
    )


def _salute_chat_targets() -> list:
    """
    Куда отправлять аудио Салюту (по порядку): запомненный личный чат →
    ...
    if ASR_SALUTE_BOT_ID and ASR_SALUTE_BOT_ID not in targets:
        targets.append(ASR_SALUTE_BOT_ID)
    return targets
...
def _relay_targets() -> list:
    ...
    if _asr_bridge.get("relay_chat_id"):
        targets.append(_asr_bridge["relay_chat_id"])
    if ASR_RELAY_CHAT_ID:
...
async def _try_relay_send(context, send_fn):
    """
    send_fn(chat_id) -> Message. Пробует все варианты чата владельца
    (в т.ч. с минусом для группового чата). Возвращает (message, None)
    или (None, описание_последней_ошибки). Рабочий chat_id запоминается.
    """
    last_err = None
    for chat_id in _relay_targets():
        try:
            m = await send_fn(chat_id)
            if m is not None and chat_id != _asr_bridge.get("relay_chat_id"):
                _asr_bridge["relay_chat_id"] = chat_id
                _asr_state_save()
                logging.info(f"ASR-мост: запомнил рабочий чат владельца (chat_id={chat_id})")
            return m, None
        except Exception as e:
            last_err = str(e)
            logging.info(f"ASR-мост: не доставил в chat_id={chat_id}: {e}")
    return None, last_err


def _asr_setup_hint() -> str:
    return (
        "1. @BotFather → /mybots → этот бот → Bot Settings → "
        "Bot-to-Bot Communication Mode → Enable\n"
        "2. Настройки → Для бизнеса → Чат-боты → этот бот → "
        "разрешить «Существующие чаты»\n"
        "3. Личный чат с @smartspeech_sber_bot должен существовать "
        "(кинь ему «привет»)"
    )


async def _notify_relay(context, text: str) -> None:
    """Сообщение владельцу моста (без parse-мода, чтобы ничего не экранировать).
    Пробует все варианты чата владельца — см. _relay_targets()."""
    if not ASR_RELAY_CHAT_ID:
        return

    async def _send(chat_id):
        return await context.bot.send_message(chat_id, text)

    sent, err = await _try_relay_send(context, _send)
    if sent is None:
        logging.warning(f"ASR-мост: не уведомил владельца: {err}")


async def _bot_file_call(method: str, fields: dict, file_field: str,
                         file_data: bytes, filename: str, mime: str) -> dict:
    """POST c файлом (multipart) — sendVoice/sendVideoNote c business_connection_id."""
    url = f"https://api.telegram.org/bot{BOT_TOKEN}/{method}"
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            resp = await client.post(url, data=fields, files={file_field: (filename, file_data, mime)})
            return resp.json()
    except Exception as e:
        logging.warning(f"Bot API вызов {method} не удался (транспорт): {e}")
        return {"ok": False, "description": str(e)}


async def _register_business_connection(context, conn) -> None:
    """Запоминаем business-подключение владельца (Telegram Business → Чат-боты)."""
    try:
        owner_id = conn.user.id if conn.user else None
        if owner_id != ASR_RELAY_CHAT_ID:
            logging.info(f"ASR-мост: игнорирую business-подключение {owner_id} (жду {ASR_RELAY_CHAT_ID})")
            return
        was_enabled = _asr_bridge["enabled"]
        _asr_bridge["connection_id"] = conn.id
        _asr_bridge["user_id"] = owner_id
        _asr_bridge["enabled"] = bool(getattr(conn, "is_enabled", False))
        _asr_state_save()
        logging.info(
            f"ASR-мост: business-подключение {'активно' if _asr_bridge['enabled'] else 'выключено'} "
            f"(connection_id={conn.id})"
        )
        if _asr_bridge["enabled"] and not was_enabled:
            await _notify_relay(
                context,
                f"{SYM['check']} Мост расшифровки подключён!\n\n"
                f"Чтобы расшифровка работала полностью автоматически:\n"
                f"{_asr_setup_hint()}\n\n"
                f"Голосовые и кружки через ссылки тогда расшифровываются сами, "
                f"без твоего участия.",
            )
        elif not _asr_bridge["enabled"] and was_enabled:
            await _notify_relay(
                context,
                f"{SYM['warn']} Мост расшифровки отключён (is_enabled=False).\n"
                f"Включи меня в Настройки → Для бизнеса → Чат-боты, и расшифровка заработает снова.",
            )
    except Exception as e:
        logging.warning(f"ASR-мост: не удалось зарегистрировать подключение: {e}")


async def asr_business_guard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Перехватчик business-апдейтов (ставится в group=-1, раньше всех).
    - business_connection → регистрируем мост;
    - business_message → ловим ответы Salute и ЗАКРЫВАЕМ апдейт
      (ApplicationHandlerStop), чтобы он не попал в обычные хендлеры.
    Обычные апдейты не трогает вообще.
    """
    conn = getattr(update, "business_connection", None)
    if conn is not None:
        await _register_business_connection(context, conn)
    bmsg = getattr(update, "business_message", None)
    if bmsg is not None:
        await _handle_business_message(context, bmsg)
...
def _enqueue_asr_job(context, *, msg_type: str, file_id: str, recipient: int,
                     db_msg_id, stored_text) -> None:
    """Ставит голосовое/кружок в очередь на расшифровку через мост."""
    global _asr_job_seq
    _asr_job_seq += 1
    job = {
        "id": _asr_job_seq,
        ...
        "recipient": recipient,
        "db_msg_id": db_msg_id,
        "stored_text": stored_text,   # что уже лежит в БД (подпись), чтобы склеить
        "mode": "business",
        "status": "queued",
        "cleanup": [],
        "context": context,
...
async def _send_manual_card(context, job) -> None:
    """
    Мост не подключён — отправляем владельцу голосовое + инструкцию.
    Расшифровка принимается ответом (reply) на карточку.
    """
    job["mode"] = "manual"
    job["status"] = "waiting_manual"
    if not ASR_RELAY_CHAT_ID:
        _asr_jobs.pop(job["id"], None)
        logging.info("ASR-мост: ASR_RELAY_CHAT_ID не задан — ручная расшифровка недоступна")
        return
    inst = (
        f"◐ Задача расшифровки #{job['id']}\n\n"
        f"1. Перешли это сообщение боту @{ASR_SALUTE_BOT}\n"
        f"2. Дождись текста (служебное «Аудиосообщение принято!» не копируй)\n"
        f"3. Ответь (reply) на ЭТО сообщение текстом расшифровки — "
        f"я доставлю её получателю и подошью в базу"
    )

    async def _send_card(chat_id):
        if job["msg_type"] == "video_note":
            m = await context.bot.send_video_note(chat_id, video_note=job["file_id"])
            try:
                m2 = await context.bot.send_message(chat_id, inst, reply_to_message_id=m.message_id)
                _asr_manual_cards[m2.message_id] = job["id"]
            except Exception:
                pass
            return m
        return await context.bot.send_voice(chat_id, voice=job["file_id"], caption=inst)

    m, err = await _try_relay_send(context, _send_card)
    if m is None:
        _asr_jobs.pop(job["id"], None)
        logging.warning(f"ASR-мост: карточка не отправилась ни в один чат владельца: {err}")
        return
    _asr_manual_cards[m.message_id] = job["id"]
    logging.info(f"ASR-мост: карточка #{job['id']} отправлена владельцу (ручной режим)")


async def _handle_manual_asr_reply(update, context, job_id: int, text: str) -> bool:
    """Владелец ответил на карточку текстом. True — апдейт обработан здесь."""
    reply_target = update.message.reply_to_message
    job = _asr_jobs.get(job_id)
    if reply_target is not None and reply_target.message_id in _asr_manual_cards:
        if job is None:
            _asr_manual_cards.pop(reply_target.message_id, None)
            await update.message.reply_text("Эта задача расшифровки уже неактивна")
            return True
        if _norm_asr_text(text) in _ASR_PLACEHOLDER_NORMS:
            await update.message.reply_text(
                f"◐ Это служебное сообщение Salute — пришли сам текст расшифровки"
            )
            return True
        for mid, jid in list(_asr_manual_cards.items()):
            if jid == job_id:
                _asr_manual_cards.pop(mid, None)
        await _complete_asr_job(context, job, text)
        try:
            await update.message.reply_text(
                f"{SYM['check']} Расшифровка #{job_id} доставлена получателю"
            )
        except Exception:
            pass
        return True
...
def link_type_body_buttons(balance=None):
    """
    Цветные кнопки В ТЕЛЕ сообщения для экрана «Создание ссылки» (выбор
    типа). Если баланс известен и его не хватает на спонсорскую — кнопка
    «Спонсорская» рендерится как disabled (<tg-button type="disabled">),
    а «Обычная» остаётся активной. На MarkdownV2-fallback остаётся
    обычная клавиатура (там кнопка спонсорской просто ведёт на экран,
    который сам проверит баланс).
    """
    can_sponsor = balance is None or balance >= SPONSOR_LINK_PRICE
    sponsor_label = f"{SYM['gift']} Спонсорская — {SPONSOR_LINK_PRICE} виол"
    if not can_sponsor:
        sponsor_label = f"{SYM['gift']} Спонсорская — не хватает виол"
    return [
        rich_button_row([
            rich_button(f"{SYM['link']} Обычная — бесплатно", data="link_type_normal", style="success"),
            rich_button(sponsor_label, data="link_type_sponsor", style="primary", disabled=not can_sponsor),
        ], align="center"),
        rich_button_row([
            rich_button(f"{SYM['cancel']} Отмена", data="main_menu", style="link"),
        ], align="center"),
    ]
...
def message_actions_keyboard(message_id, msg_type=None):
    rows = [[
    ...
    # Расшифровка по кнопке — для любого голосового/кружка, не только
    # для спонсорских ссылок (где она срабатывает автоматически).
    if msg_type in ("voice", "video_note"):
        rows.append([
            InlineKeyboardButton(f"{SYM['voice']} Расшифровать", callback_data=f"asr_go_{message_id}"),
        ])
    return InlineKeyboardMarkup(rows)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ─── Экономика (начислить/списать) — только админ, вместо /give и /take ───
            ...
            # Баланс нужен, чтобы сразу «притушить» кнопку спонсорской,
            # если на неё не хватает виол (кнопка остаётся видна, но disabled).
            balance = None
            try:
                balance = await asyncio.to_thread(vb.get_balance, user.id)
            except vb.BufferError as e:
                logging.error(f"Ошибка получения баланса для выбора типа ссылки user_id={user.id}: {e}")

            balance_line = f"\n{SYM['coin']} Ваш баланс: *{balance}* виол" if balance is not None else ""
            short = ""
            if balance is not None and balance < SPONSOR_LINK_PRICE:
                short = f"\n{SYM['warn']} На спонсорскую не хватает виол \(нужно *{SPONSOR_LINK_PRICE}*\) — кнопка недоступна\."
            text = (
                header("Создание ссылки", SYM['add']) + "\n\n"
                f"Выберите тип ссылки:{balance_line}{short}\n\n"
        ...
        elif data.startswith("asr_go_"):
            # ─── Расшифровка голосового/кружка по кнопке ───
            msg_id = safe_int(data.replace("asr_go_", ""))
            info = get_message_asr_info(msg_id) if msg_id else None
            if not info or not info[1]:
                await query.answer("Сообщение не найдено", show_alert=True)
                return
            msg_type, file_id, owner_id, stored_text = info[0], info[1], info[2], info[3]
            if msg_type not in ("voice", "video_note"):
                await query.answer("Расшифровать можно только голосовые и кружки", show_alert=True)
                return
            if user.id != owner_id and not is_admin:
                await query.answer(f"{SYM['lock']} Нет доступа", show_alert=True)
                return
            if any(
                j.get("db_msg_id") == msg_id and j.get("status") in ("queued", "sent", "waiting_manual")
                for j in _asr_jobs.values()
            ):
                await query.answer("Эта расшифровка уже в процессе")
                return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # ─── Ручная расшифровка: ответ (reply) на карточку задачи ASR ───
        # Проверяется раньше всей остальной логики: текст — это расшифровка,
        # а не команда/флоу.
        reply_target = update.message.reply_to_message
        if reply_target is not None and reply_target.message_id in _asr_manual_cards:
            _asr_job_ref = _asr_manual_cards[reply_target.message_id]
            if await _handle_manual_asr_reply(update, context, _asr_job_ref, text or ""):
                return
            ...
            if message_info:
                ...
                try:
                    ...
                    await send_rich_or_plain(
                        ...
                        body_buttons=[
                            rich_button_row([rich_button(f"{SYM['reply']} Ответить", data=f"reply_{message_id}", style="primary")], align="left"),
                        ],
                    )
                except Exception as e:
        ...
        if context.user_data.get('current_link'):
            ...
            if link_info:
                ...
                try:
                    await send_rich_or_plain(
                        context.bot, link_info[1], notification, 'HTML', message_actions_keyboard(msg_id),
                        body_buttons=[
                            rich_button_row([
                                rich_button(f"{SYM['reply']} Ответить", data=f"reply_{msg_id}", style="primary"),
                                rich_button(f"{SYM['trash']} Удалить", data=f"confirm_delete_message_{msg_id}", style="danger"),
                            ], align="left"),
                        ],
                    )
                except Exception as e:
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        stored_text = caption or transcript or None
        msg_id = save_message(link_id, user.id, link_info[1], stored_text, msg_type, file_id, file_size, file_name)
        ...
        caption_parts = []
        if caption:
            caption_parts.append(quote_html_expandable(caption_html))
        if transcript:
            caption_parts.append(f"{SYM['voice']} <b>Расшифровка:</b>\n{quote_html_expandable(esc_html(transcript))}")
        if asr_deferred:
            caption_parts.append(f"{SYM['voice']} <i>Расшифровка готовится — придёт отдельным сообщением</i>")
        user_caption = f"{icon} <b>Новый анонимный файл</b>{file_info}\n{sponsor_badge}\n" + "\n".join(caption_parts)

        try:
            await send_media_by_type(context.bot, link_info[1], msg_type, file_id, user_caption, 'HTML', message_actions_keyboard(msg_id, msg_type))
        except Exception as e:
            logging.error(f"Failed to send media to user: {e}")

        # Мост/ручная расшифровка: ставим задачу в очередь уже ПОСЛЕ доставки
        # медиа — получатель ничего не ждёт, расшифровка приедет следом.
        if asr_deferred:
            _enqueue_asr_job(
                context,
                msg_type=msg_type,
                file_id=file_id,
                recipient=link_info[1],
                db_msg_id=msg_id,
                stored_text=caption,
            )

        confirmation = f"{SYM['check']} Ваше медиа отправлено анонимно{esc('!')}"
        if transcript:
            confirmation += f"\n\n{SYM['voice']} *Расшифровка:*\n{quote(esc(transcript))}"
        if asr_deferred:
            confirmation += f"\n{SYM['voice']} Расшифровка аудио готовится{esc('.')}"
        await show_screen(update, context, confirmation, main_keyboard())
...
def main():
    ...
    # Состояние ASR-моста (@smartspeech_sber_bot) — из БД, чтобы мост
    # переживал рестарты хостинга без участия владельца
    _asr_state_load()
    ...
    # Business-апдейты (мост расшифровки через @smartspeech_sber_bot) —
