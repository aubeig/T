...
def markup_to_payload(reply_markup, disabled_callbacks=None, force_reply: bool = False):
    ...
    DisabledButton (см. disable_buttons) и поле force_reply
    ...
    Только для rich-пути (сырые HTTP-вызовы) — PTB ещё не знает про это
    поле в своих моделях, так что на обычном fallback (когда рич
    недоступен) экран просто ведёт себя как раньше, без принудительного
    ввода — это не ломает функциональность, лишь не показывает лишнее
    удобство.
    """
    data = disable_buttons(reply_markup, disabled_callbacks)
    if force_reply and data is not None:
...
async def edit_rich_message(chat_id, message_id, markdown_text: str, reply_markup=None, disabled_callbacks=None, force_reply: bool = False) -> bool:
